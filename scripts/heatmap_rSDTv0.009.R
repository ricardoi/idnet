...
sdt <- read.csv(file.choose(), header = T)

n = 21 # No. of sequences in the alignment
head(sdt, n = n)
dat = sdt


iS2mat <- function(dat = identity.scores, n= no.of.sequences){
          mat <- lower.tri(matrix(0, n, n), diag=F)
          suppressWarnings( mat[mat] <- round(dat[,3], 4)*100)
          mat[upper.tri(mat)] <-  mat[lower.tri(mat)]
          diag(mat) <- 100
          rownames(mat) = colnames(mat) = unique(dat[,1])
          return(mat)
}

sdt.mat <- iS2mat(sdt, n)


x = sdt.mat


rSDT <- function(mat = mat, size= n){
  x <- get_upper_tri(x)
  y <- melt(x, na.rm= TRUE)
...
function.rSDT(sdt, dist.dna = FALSE, dendro = FALSE)
